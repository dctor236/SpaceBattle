        ...
        if os.path.isfile(file_path):
            ...
            if suffix == '.uiprefab':
                with open(file_path,'r',encoding='UTF-8-SIG') as f:
                    newfilestr = f.read()
                    filename=filename.replace('.uiprefab','.ui')
                    newPath=os.path.join(walk_dir+"/ui/uiTemplate/perfab/", filename)
                    with open(newPath,'w+',encoding='UTF-8-SIG')as w:
                        w.write(newfilestr)

            if filename.find(".uiprefab.meta")>0:
                with open(file_path,'r',encoding='UTF-8-SIG') as f:
                    newfilestr = f.read()
                    newfilestr=newfilestr.replace('GameUI_Prefab','GameUI')
                    newfilestr=newfilestr.replace('.uiprefab','.ui')
                    namet=filename.replace(".uiprefab.meta",".ui.meta")
                    newPath=os.path.join(walk_dir+"/ui/uiTemplate/perfab/", namet)
                    with open(newPath,'w+',encoding='UTF-8-SIG') as w:
                        w.write(newfilestr)
